...
/**
 * BetterQuesting 运行时任务注入器（E4a，自 GTSWN 同名范本移植）。
 * <p>
 ...
 * 支持 GTIT 自身包与 MIAO-GTNH 综合包两个资产根；
...
public final class BqQuestInjector {
    ...
    public static void inject(String assetRoot) {
        ...
        try {
            ...
            GTInterestingThing.LOG.info("[GTIT-BQ] 任务注入完成：{} 条任务线，{} 个新任务 ({})", lines.size(), questCount, assetRoot);
        } catch (Throwable t) {
    ...
    private static int loadQuestLine(JsonObject lineSpec) {
        ...
        if (lineCreated || created > 0) {
            GTInterestingThing.LOG.info("[GTIT-BQ] 任务线 {} 装载：线新建={}，新任务={}", lineId, lineCreated, created);
        }
        return created;
    }
