...
/**
 * BetterQuesting 可选集成探测类（E4a 任务注入）。
 * <p>
 ...
 * <p>
 * 探测模式对齐仓库先例（trade/v2/NekoBqBridge.init 的 Class.forName 探测）。
 */
public final class BqCompat {
    ...
    public static void detect() {
        try {
            ...
            bqLoaded = true;
            GTInterestingThing.LOG.info("[GTIT-BQ] 检测到 BetterQuesting，任务注入集成启用");
        } catch (ClassNotFoundException e) {
            bqLoaded = false;
            GTInterestingThing.LOG.info("[GTIT-BQ] 未检测到 BetterQuesting，任务注入集成静默停用");
        }
    }
