...
/**
 * 编辑模式服务端操作处理器（O2-05 策略表化中——本类保留统一校验入口，
 * 各编辑域陆续迁出至 *EditActions 域类，经 {@link EditActionRegistry} 注册分发）
 * <p>
 * 处理 {@link NekoEditPacket} 中各类编辑操作的服务端逻辑：
 * <ul>
 * <li>交易条目编辑：{@link TradeEditActions}（O2-05 E2 已迁出）</li>
 * <li>标签页编辑：{@link PageEditActions}（O2-05 E2 已迁出）</li>
 * <li>签到奖励编辑：{@link SignInEditActions}；在线档位编辑：{@link OnlineTierEditActions}（O2-05 E3 已迁出）</li>
...
public class NekoEditActionHandler {
     ...
     * <li>ACTION 白名单：action 必须是 {@link NekoEditPacket} 已定义的 15 个常量之一</li>
